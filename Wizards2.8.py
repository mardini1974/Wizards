...
class WizardsApp (gtk.Window):
	...
	def updateValues(self):
		s.poll()
		XVal_label.set_text ("{:6.3f}".format(s.position[0]))
		YVal_label.set_text ("{:6.3f}".format(s.position[1]))  	
		return True	
	...
	def Jogaxis (self,axis,direction):
		...
		c.jog(linuxcnc.JOG_INCREMENT,False,axis,float(Feedrate_scale.get_value()),direction*float(comboincrement.get_active_text()))
		
		
		
	def on_button1_clicked (self,*args):
		s.poll()
		ValPosX.set_text ("{:6.3f}".format(s.position[0]))
	def on_button2_clicked (self,*args):
		s.poll()
		ValNegX.set_text ("{:6.3f}".format(s.position[0]))	
		
	def on_button3_clicked (self,*args):
		s.poll()
		ValPosY.set_text ("{:6.3f}".format(s.position[1]))
	def on_button4_clicked (self,*args):
		s.poll()
		ValNegY.set_text ("{:6.3f}".format(s.position[1]))	
	...
	def on_GoCenter_clicked (self,*args):
		...
		c.wait_complete()
		c.mdi("G53G0 %s"%(Centerresult.get_text().split("[")[1].split("]")[0]))
	...
	def on_GoCenterX_clicked (self,*args):
		c.mode(linuxcnc.MODE_MDI)
		c.wait_complete()
		c.mdi("G53G0%s"%(Centerresult.get_text().split("[")[1].split(" Y")[0]))
		
	def on_GoCenterY_clicked (self,*args):
		c.mode(linuxcnc.MODE_MDI)
		c.wait_complete()
		c.mdi("G53G0Y%s"%(Centerresult.get_text().split("Y")[1].split("]")[0]))
